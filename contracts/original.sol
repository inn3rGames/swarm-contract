...
contract ParimutuelBetting {
    ...
    modifier predictionEnded(uint256 _id) {
        require(block.timestamp > predictions[_id].endBidTime, "Prediction has not ended yet");
        _;
    }

    event AdminAdded(address indexed newAdmin);
    event AdminRemoved(address indexed removedAdmin);
    event PredictionCreated(uint256 id, string title, string description, string[] options, uint256 endBidTime);
    event BetPlaced(address indexed user, uint256 predictionId, string option, uint256 amount);
    event PredictionEnded(uint256 id, string winner);
    event Payout(address indexed user, uint256 amount);


    function createPrediction(string memory _title, string memory _description, string[] memory _options, uint256 _endBidTime) external onlyAdmin {
        require(_endBidTime > block.timestamp, "End bid time must be in the future");
        ...
        emit PredictionCreated(predictionCounter, _title, _description, _options, _endBidTime);
        predictionCounter++;
    }


    function placeBet(uint256 _predictionId, string memory _option) external payable predictionExists(_predictionId) predictionActive(_predictionId) {
        require(msg.value > 0, "Bet amount must be greater than zero");
        require(block.timestamp <= predictions[_predictionId].endBidTime, "Cannot place bet after bidding time has ended");
        
        Prediction storage prediction = predictions[_predictionId];
        bool validOption = false;
        for (uint256 i = 0; i < prediction.options.length; i++) {
            if (keccak256(abi.encodePacked(prediction.options[i])) == keccak256(abi.encodePacked(_option))) {
                validOption = true;
                break;
            }
        }
    ...
    function endPrediction(uint256 _predictionId, string memory _winner) external onlyAdmin predictionExists(_predictionId) predictionEnded(_predictionId) {
        Prediction storage prediction = predictions[_predictionId];
        require(prediction.isActive, "Prediction already ended");
        
        bool validOption = false;
        for (uint256 i = 0; i < prediction.options.length; i++) {
            if (keccak256(abi.encodePacked(prediction.options[i])) == keccak256(abi.encodePacked(_winner))) {
                validOption = true;
                break;
            }
        }
    ...
    function _distributeWinnings(uint256 _predictionId, string memory _winner) internal {
        Prediction storage prediction = predictions[_predictionId];
